...
class NeuralODE(nn.Module):
    def __init__(self, layers, hidden_activation, out_activation, times, training_inputs, training_outputs, test_inputs,
                 test_outputs):
        """
        Base class for NeuralODE with MLP

        :param layers (List[int]):
        ...
        self.fullinpdim = self.input_dim + self.output_dim
        self.layers = layers
        self.times = times
        self.ffnn = Net(self.fullinpdim, self.output_dim, layers, hidden_activation, out_activation)
        self.lr = 0.001
    ...
    def sortData(self, batch_len, end):
        ...
        self.training_inputs = self.training_inputs_org[:, :end].reshape(
            (self.training_inputs_org.shape[0] * no_of_time_batches, batch_len, self.training_inputs_org.shape[-1]))
        self.training_outputs = self.training_outputs_org[:, :end].reshape(
            (self.training_outputs_org.shape[0] * no_of_time_batches, batch_len, self.training_outputs_org.shape[-1]))
        self.val_inputs = self.test_inputs[:, :end].reshape(
            (self.test_inputs.shape[0] * no_of_time_batches, batch_len, self.test_inputs.shape[-1]))
        self.val_outputs = self.test_outputs[:, :end].reshape(
            (self.test_outputs.shape[0] * no_of_time_batches, batch_len, self.test_outputs.shape[-1]))
    ...
    def train(self, cut, step, epochs, set_optimizer='adam', lr=0.001, solver='euler', batch_size=32, plot_loss=True):
        ...
        training_inputs = self.training_inputs_org[:, :cut:step]
        training_outputs = self.training_outputs_org[:, :cut:step]
        
        dataset = TimeSeriesDataset(training_inputs, training_outputs)
        dataloader = DataLoader(dataset, batch_size=batch_size, shuffle=True)
        self.solver = solver
        print(
            f" Training started with {self.training_inputs.shape[0]} training batches, and {self.training_inputs.shape[1]} time steps.")
        for epoch in range(epochs):
            ...
            for batch_inputs, batch_outputs in dataloader:
                ...
                if set_optimizer == 'lbfgs':
                    ...
                    def closure():
                        ...
                        model_out = odeint(deriv_approx, batch_outputs[:, 0, :], times, method=solver).permute(1, 0, 2)
                ...
                if set_optimizer == 'adam':
                    self.optimizer = optim.Adam(self.ffnn.parameters(), lr=lr, betas=(0.9, 0.999), eps=1e-7,
            ...
            self.ffnn.eval()
            with torch.no_grad():
                self.testpredict = self.predict(times, self.val_inputs[:, :cut:step], self.val_outputs[:, 0, :],
                                                numpyarr=False)
                valloss = self.criterion(self.testpredict, self.val_outputs[:, :cut:step])
                self.validation_loss += [valloss.detach().numpy()]

            print(
                f'Epoch: {epoch + 1}/{epochs}, loss: {np.round(self.training_loss[-1], 6)}, val loss: {np.round(self.validation_loss[-1], 6)}, {np.round(endepoch - startepoch, decimals=3)}s/epoch')

        end = time.time()
        self.training_time += np.round(end - start, decimals=1)
        print(
            f'Final loss: {np.round(loss.detach().numpy(), 6)}, training duration for {epochs} epochs: {self.training_time} seconds')
    ...
    def predict(self, times, inps, init, solver='euler', print_time=False, numpyarr=True):
        ...
        startpred = time.time()
        pred = odeint(deriv_approx, init, times, method=solver).permute(1, 0, 2)
        endpred = time.time()
    ...
    def plot_loss(self):
        ...
        plt.semilogy(self.validation_loss, c='red');
        plt.title(f'NeuralODE: {str(self.layers)}, solv.: {self.solver}, train_time: {self.training_time} s')
        plt.xlabel('epochs');
        plt.ylabel('MSE');
